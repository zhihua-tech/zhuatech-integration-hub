...
package cn.zhuatech.integrationhub.config;
import cn.zhuatech.integrationhub.domain.DomainCatalog;
import cn.zhuatech.integrationhub.model.*;
import cn.zhuatech.integrationhub.repository.*;
...
@Configuration public class DataInitializer {
 @Bean CommandLineRunner seed(BusinessRecordRepository records,SystemSettingRepository settings,DomainCatalog catalog) {
  return args -> { if(records.count()>0)return;
            ...
            settings.save(new SystemSetting("recordNoRule", "INTHUB-{YYYY}-{SEQ}"));
   ...
   int sequence=1; for(var module:catalog.modules()) {
    String no="INTHUB-DEMO-"+String.format("%03d",sequence);
