...
| POST | `/domain/decision` | 执行企业集成平台专属领域规则 |
...
| 字段 | 类型 | 含义 |
| --- | --- | --- |
| `flowCode` | String | 集成流程编码 |
| `sourceRecords` | int | 源记录数 |
| `deliveredRecords` | int | 成功投递数 |
| `failedRecords` | int | 失败记录数 |
| `p95LatencyMs` | int | P95延迟(ms) |
| `duplicateEvents` | int | 重复事件数 |
| `mappingValidated` | boolean | 映射规则已验证 |
| `authConfigured` | boolean | 连接认证已配置 |
| `deadLetterCleared` | boolean | 死信已处置 |
